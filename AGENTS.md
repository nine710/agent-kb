...
- Git 操作仅在用户明确要求时执行 commit/push；本仓库当前非 git 仓库，需要时先 `git init` 并配置远端 `nine710/agent-kb`。
- v0 不做的事：消费侧 design-time skill（用户自行构建）、个人经验入卡（永久不做）。
- `SCHEMA.md`、`scripts/validate_card.py`、`cards/` 等是 `agent-kb计划.md` Phase 1 中将要创建的文件，目前在仓库内尚不存在。
