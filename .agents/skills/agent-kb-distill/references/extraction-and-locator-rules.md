...
| Git repository document | repository-relative path plus heading plus commit | high | path plus line range plus commit |
| Code | repository-relative path plus symbol or line range plus commit | high | file path plus line range plus commit |
...
- Read [material profile contracts](material-profile-contracts.md) after profile selection. It defines the inventory unit, required quality check, and evidence limits for each material type.
